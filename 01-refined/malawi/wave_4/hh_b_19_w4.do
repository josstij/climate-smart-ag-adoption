...
* open log
	cap log close
	log using "$logout/hh_mod_b_19_w4", append
...
* merge data
	...
	merge		1:1 PID using "$root/hh_mod_e_19"
	drop		_merge
...
* replace edu
	replace		edu = 0 if edu == . & (hh_c05_1 == 2 | hh_c06 == 2)
				
	keep		y4_hhid indiv_num sex edu age relate mrry wage nfe farm


************************************************************************
**# 2 - create standard indiv ID
************************************************************************
...
* Identify any duplicates created by slicing the PID
	duplicates 	tag y4_hhid indiv, gen(dup)
...
* save file
	isid		y4_hhid indiv
	qui: 		compress
	save 		"$export/hh_mod_b_19_w4", replace
