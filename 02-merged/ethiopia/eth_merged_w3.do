...
* define paths
	global 		root 	"$data/lsms_gender_data/01-refined_data/ethiopia/wave_3"
	global 		export 	"$data/lsms_gender_data/02-merged_data/ethiopia/wave_3"
	global 		logout 	"$data/lsms_gender_data/02-merged_data/ethiopia/logs"	

* open log 
	cap 		log 	close
	log 		using 	"$logout/eth_merged_w3", append
...
* merge edu
	merge 1:1 	household_id individual_id using "$root/sect2_hh_w3"
	*** 23,385 matched, 4,594 not matched from master
	
	drop		_merge
	
* merge work
	merge 1:1 	household_id individual_id using "$root/sect4_hh_w3"
...
************************************************************************
**## 2.1 - merge mgmt and dejure (sect2_pp_w3) - parcel 
************************************************************************
...
* merge each file for each mgmt1
	merge 1:m 	household_id mgmt1 using "$root/sect2_pp_w3"
	*** only match 13765, not matched 22461 from master and 48 from using
	*** this is fine because we are JUST matching for manager 1, in s2_pp_w3
...
*merge each file for each deju
	merge 1:m 	household_id deju`i' using "$root/sect2_pp_w3"
	*** matched 2,512
...
************************************************************************
**## 2.2 - merge mgmt (sect3_pp_w3) - field
************************************************************************
...
*merge each file for each deju
	merge 1:m 	household_id mgmt`i' using "$root/sect3_pp_w3"
	*** matched 2,780
...
*merge each file for each monman
	merge 1:m 	household_id monman`i' using "$export1/sect9_ph_w3"
...
* compress and save final merged file 
	qui:		compress
	isid		household_id individual_id parcel_id holder_id 
	save		"$export/mm_demo_w3", replace
*/
************************************************************************
**# 4 - end matter
************************************************************************
