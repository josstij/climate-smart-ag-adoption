...
* define paths
	global 		root 	"$data/lsms_gender_data/01-refined_data/ethiopia/wave_2"
	global 		export 	"$data/lsms_gender_data/02-merged_data/ethiopia/wave_2"
	global 		export1 "$data/lsms_gender_data/01-refined_data/ethiopia/wave_2"
...
* open log 
	cap 		log 	close
	log 		using 	"$logout/eth_merged_w2", append
...
* check unique id
	isid		household_id2 individual_id2
	
* merge edu
	merge 1:1 	household_id2 individual_id2 using "$root/sect2_hh_w2"
	*** 23,785 matched, 2373 not matched, all from master
	*** in the section 1 hh roster of w2, when we match section 2 - we lost 2373 
	*** merging  in edu from sec2, means that we are missing education for 2373 people, which is okay
	
	drop		_merge
	
* merge work
	merge 1:1 	household_id2 individual_id2 using "$root/sect4_hh_w2"
	*** 23785 matched, 2373 not matched, all from master
	*** nice, matches the same when adding work, as when adding in education
...
************************************************************************
**## 2.1 - merge mgmt and dejure (sect2_pp_w2) - parcel 
************************************************************************
...
* merge each file for each mgmt1
	merge 1:m 	household_id2 mgmt1 using "$root/sect2_pp_w2"
	*** only match 13765, not matched 22461 from master and 48 from using
	*** this is fine because we are JUST matching for manager 1, in s2_pp_w2
...
*merge each file for each deju
	merge 1:m 	household_id2 deju`i' using "$root/sect2_pp_w2"
	*** matched 2,512
...
************************************************************************
**## 2.2 - merge mgmt (sect3_pp_w2) - field
************************************************************************
...
*create mgmt ID
	gen			mgmt`i' = indiv_num
	
*merge each file for each deju
	merge 1:m 	household_id2 mgmt`i' using "$root/sect3_pp_w2"
	*** matched 2,780
...
*merge each file for each monman
	merge 1:m 	household_id monman`i' using "$export1/sect9_ph_w2"
...
* rename demographic variables to monman prefix
	...
	isid        holder_id household_id2 parcel_id field_id crop_code
...
* compress and save final merged file 
	qui:		compress
	isid		household_id individual_id parcel_id holder_id 
	save		"$export/mm_demo_w2", replace
*/
************************************************************************
**# 4 - end matter
************************************************************************
