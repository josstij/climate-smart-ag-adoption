...
* define paths
	global 		root 	"$data/lsms_gender_data/01-refined_data/nigeria/wave_1"
	global 		export 	"$data/lsms_gender_data/02-merged_data/nigeria/wave_1"
	global 		logout 	"$data/lsms_gender_data/02-merged_data/nigeria/logs"

* open log 
	cap 		log 	close
	log 		using 	"$logout/nga_merged_w1", append
...
* merge edu
	merge 1:1 	hhid indiv using "$root/sect2_plantingw1"
	*** 24,466 matched, 4206 not matched, all from master
	*** all but 35 unmatched are younger than 5
...
* merge work
	merge 1:1 	hhid indiv using "$root/sect3_plantingw1"
	*** 24,260 matched, 4412 not matched, all from master
	*** all but 52 unmatched are younger than 5
...
************************************************************************
**## 2.1 - merge money manager (sect11h_plantingw1)
************************************************************************
...
* loop through money managers
	...
	* merge each file for each money manager
		merge 1:m 	hhid monman`i' using "$root/sect11h_plantingw1"
...
* loop for mgmt 1 - 5
    ...
    * merge with the agricultural file
		merge 	1:m hhid mgmt`i' using "$root/sect11b_plantingw1"
...
* merge each file for each manager
	merge 	1:m hhid deju1 using "$root/sect11b_plantingw1"
...
* check unique id
	isid 	hhid plotid deju1

* save file
	qui: 	compress
	isid 	hhid plotid
	save 	"$export/deju1", replace
...
* merge with the agricultural file
	merge	1:m hhid defa`i' using "$root/sect11b_plantingw1"
...
************************************************************************
	...
	merge		1:1 hhid plotid using "$export/deju1", gen(_deju1)
