...
* define paths
	global 		root 	"$data/lsms_gender_data/01-refined_data/nigeria/wave_3"
	global 		export 	"$data/lsms_gender_data/02-merged_data/nigeria/wave_3"
	global 		logout 	"$data/lsms_gender_data/02-merged_data/nigeria/logs"

* open log 
	cap 		log 	close
	log 		using 	"$logout/nga_merged_w3", append
...
* merge edu
	merge 1:1 	hhid indiv using "$root/sect2_harvestw3"
...
* merge work
	merge 1:1 	hhid indiv using "$root/sect3_plantingw3"
...
************************************************************************
**## 2.1 - merge money manager (sect11k_plantingw1)
************************************************************************
...
* loop through money managers
	...
	* merge each file for each money manager
		merge 1:m 	hhid monman`i' using "$root/sect11k_plantingw3"
	
	* rename monman1 demo vars	
		local 		demo relate sex age mrry visit edu wage farm nfe
...
* loop through money manager files
	...
	* merge monman files
		merge 1:1 	hhid prod_cd using "`monman`i''", nogen
	}

* save money manager file
	qui: 		compress
	isid 		hhid prod_cd
	save 		"$root/monman", replace
...
* loop for mgmt 1 - 6
    ...
    * merge with the agricultural file
		merge 	1:m hhid mgmt`i' using "$root/sect11b1_plantingw3"
...
* merge each file for each manager
	merge 	1:m hhid deju`i' using "$root/sect11b1_plantingw3"
...
* loop over managers 1 to 3 and merge sequentially
	forvalues 	i = 1/3 {
...
* merge with the agricultural file
		merge	1:m hhid defa`i' using "$root/sect11b1_plantingw3"
