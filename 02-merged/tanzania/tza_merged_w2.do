...
* define paths
	global			root 	"$data/lsms_gender_data/01-refined_data/tanzania/wave_2"
	global			export 	"$data/lsms_gender_data/02-merged_data/tanzania/wave_2"
	global			logout 	"$data/lsms_gender_data/02-merged_data/tanzania/logs"

* open log
	cap		log		close
	log				using 	"$logout/tza_merged_w2", append
...
* loop for mgmt 1 - 3
    ...
    * merge with the agricultural file
		merge			1:m hhid mgmt`i' using "$root/AG_SEC3A_w2"
...
* merge each file for each manager
		merge			1:m hhid deju`i' using "$root/AG_SEC3A_w2"
...
* merge with the agricultural file
		merge			1:m hhid defa`i' using "$root/AG_SEC3A_w2"
...
* save final merged file
	...
	count
	isid			hhid plotnum
	save			"$export/tza_merged_w2", replace
