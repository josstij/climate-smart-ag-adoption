...
* define paths
	global			root 	"$data/lsms_gender_data/01-refined_data/tanzania/wave_7"
	global			export 	"$data/lsms_gender_data/02-merged_data/tanzania/wave_7"
	global			logout 	"$data/lsms_gender_data/02-merged_data/tanzania/logs"

* open log
	cap		log		close
	log				using 	"$logout/tza_merged_w7", append
...
* loop for mgmt 1 - 3
    ...
    * merge with the agricultural file
		merge			1:m hhid mgmt`i' using "$root/AG_SEC_3A_w7"
...
* merge manager files
	use				"$root/AG_SEC_3A_w7", clear

* loop over managers 1 to 4 and merge sequentially
	forvalues 	i = 1/3 {

		merge			m:1 hhid plot_id using "`mgmt`i''", nogen
	}
...
* merge each file for each manager
		merge			1:m hhid deju`i' using "$root/AG_SEC_3A_w7"
...
* loop over managers 1 to 2 and merge sequentially
	forvalues 	i = 1/2 {
		...
		merge			m:1 hhid plot_id using "`deju`i''", nogen
	}
...
* merge with the agricultural file
		merge			1:m hhid defa`i' using "$root/AG_SEC_3A_w7"
...
* loop merge owner files
	forvalues 	i = 1/2 {
		...
		merge			m:1 hhid plot_id using "`defa`i''", nogen
	}
...
* save final merged file
	tostring		plotnum, replace
	qui:			compress
	count
	isid			hhid plotnum
	save			"$export/tza_merged_w7", replace
