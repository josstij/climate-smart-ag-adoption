...
* define paths
	global			root 	"$data/lsms_gender_data/01-refined_data/tanzania/wave_5"
	global			export 	"$data/lsms_gender_data/02-merged_data/tanzania/wave_5"
	global			logout 	"$data/lsms_gender_data/02-merged_data/tanzania/logs"

* open log
	cap		log		close
	log				using 	"$logout/tza_merged_w5", append
...
* loop for mgmt 1 - 3
    ...
    * merge with the agricultural file
		merge			1:m hhid mgmt`i' using "$root/AG_SEC_3A_w5"
...
* merge each file for each manager
		merge			1:m hhid deju`i' using "$root/AG_SEC_3A_w5"
...
* merge with the agricultural file
		merge			1:m hhid defa`i' using "$root/AG_SEC_3A_w5"
...
* save final merged file
	...
	count
	isid			hhid plotnum
	save			"$export/tza_merged_w5", replace
