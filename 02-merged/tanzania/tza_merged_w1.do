...
* define paths
	global			root 	"$data/lsms_gender_data/01-refined_data/tanzania/wave_1"
	global			export 	"$data/lsms_gender_data/02-merged_data/tanzania/wave_1"
	global			logout 	"$data/lsms_gender_data/02-merged_data/tanzania/logs"

* open log
	cap		log		close
	log				using 	"$logout/tza_merged_w1", append
...
* load individual roster
	use				"$root/SEC_B_C_D_E1_F_G1_U_w1", clear
...
* loop for mgmt 1 - 3
    ...
    * merge with the agricultural file
		merge			1:m hhid mgmt`i' using "$root/SEC_3A_w1"
...
* merge each file for each manager
		merge			1:m hhid deju`i' using "$root/SEC_3A_w1"
...
* merge with the agricultural file
		merge			1:m hhid defa`i' using "$root/SEC_3A_w1"
...
* save final merged file
	...
	count
	isid			hhid plotnum
	save			"$export/tza_merged_w1", replace
